...
class SecurityHubIngester(object):
    ...
    _log = None
        ...
        # Add this method inside the SecurityHubIngester class.
    def ingest_findings(self, event):
        # Your logic for processing the event and extracting the findings goes here.
        # For this example, I assume that the event contains a list of findings.
        ...
        # Check the response for any failed findings
        failed_count = response.get('FailedCount', 0)
        if failed_count > 0:
            failed_findings = response.get('FailedFindings', [])
            self._log.error(
                f"Failed to import {failed_count} findings: {failed_findings}")

        return f"Ingested {len(findings) - failed_count} findings successfully, {failed_count} failed."
